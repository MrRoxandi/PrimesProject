...
public:
    static std::array<cpp_int, 2> createPair(const size_t bit_count) {
        ...
        do { second = primes::getRandomPrime(bit_count); } while (first == second);
        return std::array{ first, second };
    }
    static std::array<cpp_int, 2> createPair(const cpp_int& lower_bound, const cpp_int& upper_bound) {
        cpp_int first, second;
        first = primes::getRandomPrime(lower_bound, upper_bound);
        do { second = primes::getRandomPrime(lower_bound, upper_bound); } while (first == second);
        return std::array{ first, second };
    }

    static std::array<cpp_int, 2> createInversePair(const cpp_int& modulus) {
        cpp_int first, second;
        do { first = integers::getRandomInteger(1, modulus - 1); } 
    ...
    cpp_int getKey(const uint8_t& n = 1) const {
        return (n == 0) ? _d : _c;
    }
    cpp_int getN() const {
        return _n;
